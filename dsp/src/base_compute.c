...
void Base_Compute(float NOMINAL_PHASE_VOLTAGE,float NOMINAL_PHASE_CURRENT,
		  ...
		  float *SPD_CTRL_BANDWIDTH, float *BASE_ANGULAR_FREQUENCY)
    {
        ...
        *VOLTAGE_BASE = NOMINAL_PHASE_VOLTAGE/Ubase;
        *VOLTAGE_MAX = 1.05*(*VOLTAGE_BASE);
        *VOLTAGE_MIN = -1.05*(*VOLTAGE_BASE);
        *MAX_CURRENT = 1.05*(NOMINAL_PHASE_CURRENT/Ibase);
        *NOM_CURRENT = 0.6804;
        *MIN_CURRENT = 0.1*(*NOM_CURRENT);
        *CURR_CTRL_BANDWIDTH = 2.1972/(RISE_TIME_CC/tbase);
        *SPD_CTRL_BANDWIDTH = (*CURR_CTRL_BANDWIDTH)/20;
	*BASE_ANGULAR_FREQUENCY = (2*PI*NOMINAL_STATOR_RESISTANCE)/wbase;
    }
