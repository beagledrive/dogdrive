...
void RG_Controller(RG_Typedef *RG_Struct,float Vd_ref,float Vq_ref,float Wr_ref,float Wr,float w1,float *Id_ref,float *Iq_ref)
{
    ...
    // Here an integral control is implemented for field weakening as given in the course book
    ...
    *Id_ref = Id_cur;
    ...
    if(AW_Flag == 0)
    {
        int_s_cur = PI_Struct->int_s_pre + PI_Struct->Ts*Err_d;
    }
